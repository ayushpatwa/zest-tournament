...
/**
 * Checks if today's and tomorrow's (if >= 11:00 PM) 1v1 schedule exists,
 * and automatically heals / generates if missing.
 */
export const checkAndAutoGenerateDailyMatches = async () => {
  try {
    ...
    const todayQuery = query(tourneysCol, where("matchDate", "==", today));
    const todaySnap = await getDocs(todayQuery);

    let todayDailyCount = 0;
    todaySnap.forEach(d => {
      const data = d.data();
      if (data.isDailyScheduled || d.id.startsWith(`daily_${today}`)) {
        todayDailyCount++;
      }
    });

    if (todayDailyCount < 100) {
      console.log(`[AutoScheduler] Today (${today}) has ${todayDailyCount}/100 1v1 matches. Auto-generating...`);
      await generateDaily1v1Matches(today, false);
    }

    // Check if current time is past 11:00 PM (23:00)
    ...
    if (currentHour >= 23) {
      ...
      if (tomorrowDailyCount < 100) {
        console.log(`[AutoScheduler] Past 11:00 PM and tomorrow (${tomorrow}) has ${tomorrowDailyCount}/100 matches. Auto-generating...`);
